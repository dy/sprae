...
directive.with = (el, evaluate, rootState) => {
  ...
  return () => {
    ...
    console.group('with')
    let s = store(evaluate(rootState), rootState)
    ...
    // for (let k in rootState) rootState[k] // touch parent signals
    // for (let k in s) s[k] // touch own signals

    console.groupEnd('with')
