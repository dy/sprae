...
- sparse navigation opposite it (Usage, Reference, FAQ, capitalized), as labels, not buttons, each with the shared link rule (`--link-rule`, currentColor at 28%: a faint 1px underline `0.3em` below the text that fills on hover with no transition, one token for every text link on the page: menu, feature links, Start, FAQ, footer; every text link is also one weight step above its text, 500; stat numerals are links without an underline), with the version stamp, then npm's and GitHub's marks, one colour and one size (`currentColor`, 24px in 44px targets): the stamp is the caption font at link weight (500, never mono) with the shared link rule, linking the package's version history on npm; the npm mark links the package, the GitHub mark the repository, and the last glyph sits on the shell's right edge (the tap target overhangs). On phones the two marks narrow their targets to 32px (still 44 tall); below 27rem the version stamp leaves the row, and below 22.5rem the npm mark does too (the footer keeps both links) while the links close up a step, so the row holds down to 320px. The stamp and the two pinned script sources must name the version npm serves; the footer's third column links npm, JSR (`@dy/sprae`), GitHub and Releases
