...
- The hero phrase plays on its own, in a loop (`play: 'cycle'` in the hero scope, `director()`): rest, the attribute names light, the attributes evaporate, hydration types a fresh saying, it stays readable for a few seconds, the source condenses back, and the cycle repeats with a new saying. The tempo lives in one `window.tempo` table (attrs 1.4s, evaporate 1.8s, typing lead 2s at 70ms a key, read 3s, rest 2.2s, back 1.2s), and the hook words' `--step` underlines follow it. A hovered hook word still plays its own step and holds it; leaving it resumes the loop. The plate stills the loop while a fine pointer rests on it (`over`) or while anything inside the live frame holds focus (`focus`, reported by the frame's own document through `attend()`, since frame focus never reaches the page); both clear `mode`, so typing stops at once and the visitor can type. The loop resumes from wherever it was stopped, condensing a hydrated plate back first. The reset is itself visible: the values blur away, the attributes open back up at the forward pace (0.8s, the `restoring` phase lasting as long), and what was typed fades out of the live textarea (`color` transition in the frame, 400ms) before it says `Hello.` again, done by `typewrite()`'s own teardown, unless someone typed there meanwhile. Each hook word carries one underline, a transition, not keyframes: it fills from the left over `--sync` (the plate's own transition speed for that step) at full ink and then simply holds while the step runs; when the step hands over it draws back and fades over 0.45s from wherever it got to, so an interrupted step never snaps. The phrase is no longer a control: no pointer cursor, no tap toggle. Under reduced motion the loop does not start.
- The three hero texts are printed into the wash: `--ink-print: #28418e` under `mix-blend-mode: multiply`, which multiplies to exactly `--ink` on the paper and deepens where the bloom is. For that the hero and its shell form no stacking context; the wash (`z-index: -3`) and corner bands (`-2`) sit below the flow, the feature stage is a plain in-flow block whose paper paints under any positioned element (a negative z-index there put the transparent `main` above it for hit-testing and killed its links), the transition rule is painted before the shell, and the plate isolates its own layers.
- The “NOT A FRAMEWORK” label arrives letter by letter: `stack()` renders each letter in its own inline box with its index, and each settles out of a 6px blur, like the titles, dropping a third of an em down the page onto the letters before it (the span is turned around, so the drop is a local rise), 0.8s each, 60ms apart in reading order from 300ms; the rule under it draws once they have landed (1.4s). The label is set light (300) with NOT alone bold. The letters are held paused with the rest of the hero until the first frame has painted, and simply show under reduced motion.
- Retain the left-edge “NOT A FRAMEWORK” annotation and existing hero transition. Below 64rem the hero line reaches the left edge, so the annotation stands at the right shell edge instead, its rule on the inner side. On phones the hero is left-aligned and smaller, and the shell is a two-column grid: the texts in the first column, the annotation vertical, small and without its rule in a narrow second one, its foot a step (`--sp-1`) above the slogan's box so it sits level with the last line's letters; the plate spans both. Nothing is measured for it.
...
on the supplied component; there is no noise toggle: the bloom carries its own grain. The page grain blends as `overlay` at 0.2, never `luminosity`: overlay textures without moving the mean tone, so every navy surface below the hero (the usage code box, the dark reference) renders the plate's exact `--plate-solid`; the luminosity pass lifted them by some 20 levels. The background displays even with JavaScript disabled.

The two bloom blues are the page's dynamic motive beyond the hero. Seen through paper (`--glow-blue`, `--glow-cyan`: each mixed 70/30 with `--paper` in oklch, so they read less saturated than the raw bloom), they form the external glow behind the code surfaces: a blurred conic ring (`--glow`, blue → cyan → blue → cyan) on a `::before` under the plate and the usage code box, extended `--sp-3` beyond the box, dropped one step (`translate: 0 var(--sp-3)`) so it reads as a lit shadow rather than a halo, blurred `--sp-6`, at 0.32 opacity (0.6 while the surface holds focus), turning once every 24s on the registered `--glow-angle`. A single navy drop shadow (`--shadow-plate`) keeps the surfaces grounded. The plate isolates its layers (`.plate` host, `.plate__body` surface, the glow between); the usage panel isolates for its code box, whose clipping moved to `.urow` so the ring is not cut. The same blues light the stat divider above a linked figure under a pointer. A blue mist (`--mist`) rises from the bottom-left of the usage block and the footer and wanders on a registered `--mist-x` (12% ↔ 28%, 30s); the usage block's ellipse is bounded (60rem × 28rem) so it stays a corner of the taller section; both sit on the same `--paper`. All of it rests under reduced motion.

The tridot mark's three dots are an equilateral triangle (side 81.4 in the 148 box, apex up, its bounding box centred, so the centroid sits at 74 85.75); the mark turns on its own about that centroid, one turn in 40s, in the top bar and the footer (inline SVG, `.mark`), and the favicon turns with it: the same three dots drawn to a 64px canvas, centroid at the centre, at the mark's current animation angle, a step every 250ms while the tab is on screen (rAF), Safari keeping the static SVG. `assets/logo.svg` carries the same geometry. Under a pointer or focus the dots keep swinging in toward each other and back, the breathing mode of a triatomic molecule: a 1.6s oscillation (`--osc`, a registered number on keyframes) runs underneath all the time, and hover springs its amplitude (`--amp`, transitioned on the damped `linear()` spring `--ease-spring`, 16% overshoot, bezier fallback) from 0 to 1, leaving springs it back, so nothing snaps at either end; `--breath = 1 − amp · osc · 0.28` scales the group about the centroid to 0.72 at the trough while each dot counter-scales and keeps its size, so the three stay equidistant throughout. No rotation or turning icon under reduced motion.
...
The corner paper fades reach upward at ±30°. The hero transition rule paints above them and above the feature paper, under the plate and its glow (it is placed before the shell in the source, both at `z-index: auto`), with the edge label above everything. Render the
...
beside each figure carries the meaning, and the `dt` labels serve assistive technology only. A single-line unit (deps, directives, modifiers) sits exactly on the numeral's baseline; two-line units centre on it and drop 2px to sit optically level (the drop is theirs alone). Each cell reaches from the rule above the strip to the rule below it (negative block margins give the room back), so a pointer anywhere between the two rules is on the stat; the cells sit at `z-index: 1`, above the Usage section that follows, whose paper would otherwise cover the glare on its rule. A stat that leads somewhere (speed, directives, modifiers: `div:has(a)`) lights both rules under a pointer or focus: `--rule-size` hairlines in `--ink` centred on the figure and fading out at their ends, the nearer rule's the wider: `glare()` on the strip (`:onpointermove.throttle-raf`) writes the pointer's place between the rules to `--y` on the cell, and the two glares scale to `1.4 - y` and `0.4 + y`. Every value is a number in the same type role. No inter-stat rules.
...
- Five named contexts: HTML, ESM, JSX, Markdown, Templates. Natural-width text tabs wrap
  ...
  a single row. On phones the strip is one row that scrolls sideways with no scrollbar, bleeding into the gutters; its edges fade toward where more lies (`--fade-l`/`--fade-r`, driven by a scroll timeline, the same mechanism as the aspects strip), so no fade shows when nothing is beyond. The Usage block shows no line numbers (the comparison keeps its own: counting lines is what it compares) and leaves no gutter for them: the source and the copy status under it sit on one `--usage-inset` (`--sp-6`; `--sp-5` on tablets and phones).
- Source is deliberately formatted with shorter lines and wraps in place on desktop; on phones code never wraps: the plate, the Usage block, and the reference example scroll sideways under a thin translucent bar (`scrollbar-color: var(--plate-edge) transparent`). Height follows content. Keep concise operational notes below.
...
No synthetic drag or extra navigation controls; the only clone is the marquee's second copy.

Reference keeps the four-column index (three on tablets, two on phones), light directive names and
descriptions, and underline tabs in flow. The index is four columns on desktop (three of `shell/4` less the gutter and one of `shell/4`, so the third column starts on column line 5) and three on tablets, with 14px descriptions set 2px under the names; the section is a navy plate: `.reference` inverts its tokens (`--ink`, `--rule`, `--surface`, `--code-accent`, `--focus`) to the code surfaces' on `--plate-solid`, inline code in the intro and the descriptions takes the directive colour, and the plate's dot grid fills everything from a unit under the tabs down: `.ref-box` bleeds out of the shell to the screen edges (`margin-inline: calc(50% - 50vw)`, padded back in) and down to the section's foot (its bottom padding is the section's `--ref-pad`, its bottom margin the negative of it), with `background-position-x: calc(50vw - shell / 2)` so the first dot column sits on the names' left edge and, padded `--sp-8` on top, a dot row on every list row: rows are on a three-unit pitch (`grid-auto-rows: minmax(96px, auto)`, no row gap, entries content-sized at the top of their row; phones keep the compact auto rows) and an entry is exactly two units, 64px (8px pads, a 24px name line, a 4px gap, a 20px description line), its hover card half a unit wider on each side; the preview stays the dark card, with a hairline edge; the preview dialog's hidden heading reads "Example" until an entry names it, never empty; the intro follows the active tab: one sentence each, for directives "An element can have multiple directives, initialized in order – `:if :each` is not the same as `:each :if`. Combined `:id:name="field"` sets both from one expression", for modifiers "Directives can have modifiers applied left to right – `:onkeydown.ctrl-s.prevent` key-gates then prevents the default then runs expression". The two should run the same number of lines; both are rendered in one grid cell with the inactive one invisible, so the block keeps the taller height and the rule below never moves when the tab changes. Comments in examples precede the line they describe, never trail it, so nothing wraps. Columns are separated by a `--sp-10` gap and rows by the empty rest of their 96px pitch; a pointer resting there (250ms) closes the preview, so one entry can be left without landing on its neighbour, while passing through to another entry keeps it open and morphs it: the code crossfades, the height eases, the box glides. The kind tabs use a plain space before their counts. Match the
...
a 300ms fade/scale, and 260ms blurred content transitions. Every source swap (the preview, and the Usage and Comparison panels) crossfades through `swap()`: a ghost of the old markup blurs out (220ms) over the new markup blurring in (260ms), with no scaling, while the box height eases between the two (260ms); a swap arriving mid-ease cancels the running ease before measuring, so it starts from the height shown and lands on the natural one. The tab's `:onclick` drives it, with the code element captured by `:ref`. Highlight the hovered tile with a white card only (`#fff`, lighter than the ground, never darker); its name carries no underline, since the moving underline belongs to tab strips. Do not append an arrow. Hover previews ignore pointer events, so they
do not obstruct the index; there is no dialog header. Code replaces the reference image
and uses a wider, content-sized dark surface. One interaction for every pointer: hovering shows the preview trailing the pointer; a click, a tap, or keyboard focus pins it for reading, selection, and scrolling (pinning what is already shown keeps it where it is; pinning another entry morphs it there); a click on the pinned entry, a click outside, Escape, a scroll, or a tab change closes it, with no close control. The popover is `manual`, so the browser never light-dismisses it under a click on an entry; outside clicks and Escape are handled by the page.
...
Both retain line numbers using the shared source-row renderer. Below 48rem the section is not shown.
...
link labelled “Sponsor Sprae,” and a compact navigation row. A soft blue wash returns at the lower left, the same drifting mist the usage block carries. All footer
...
type role. Separate MIT and the omkara with spacing, without a dot; both are links without an underline, MIT to the licence file, the omkara to the krishnized licence. The footer line sits on the ten-column grid: the mark at column 0, the links (Usage, Reference, FAQ, GitHub) one per column from line 4, where the note above starts, the licence at the right edge; below 48rem the mark takes its own row and the links share the next row with the licence. The cells are exactly three grid columns wide with half-column gutters (`3 × shell/10`, gap `shell/20`), so they start on column lines 0, 3.5 and 7 at every width, tablets included; no per-cell nudges. The three proofs are never narrowed to quarters.5, 5, 7.5 (15rem on tablets and phones), 16px titles, 14px copy held to two lines, a tl;dr each: no repetition of a proof or another aspect, nothing obvious, optional code) runs leftward on its own at 20px/s as a marquee that keeps the native scroll: the strip holds the list twice (the copy `inert`, so it is neither read nor tabbed), and `wrap()` on scroll keeps the position inside one span (the distance from an item to its copy: past it the scroll jumps back a span, at the start it jumps forward one, the same picture either way, the jump points strictly apart so one landing never triggers the other), so the drift and the visitor's own scrolling go round without an end in either direction; it stands still while a pointer or keyboard focus rests on it (`drift(el)` from `:fx`, paused by `still` in the viewport scope; none under reduced motion); it fades out past the shell edges, so it reads as an index, not as a second row of features (on phones the fade is deeper, 3rem, and follows the scroll position: only the side with more content fades). Code in an aspect must not wrap at 15rem; an aspect may carry one text link where a companion project is the real answer (Custom elements → define-element), set in the aspect's own caption font; Pluggable signals carries, instead of code, a two-column table of the alternatives (name, a link where there is one; size), the readme's list without its notes. The strip scrolls sideways only: vertical overflow is hidden, so a wheel over it moves the page. The strip carries a visually hidden h2 "Aspects", so its h3s do not read as Usage subsections. Order the aspects by importance to someone deciding: the CSP build first with browser extensions as its use, server components (no `'use client'`), then fragments, event chains, async expressions, optional keys, pluggable signals with the Preact-signals build as its ready case; then modifiers (timing, key filters), lifecycle (mount, cleanup, refs, spread), custom directives; the state notes later (computed values, stores, untracked `_` props, the signals API, signals in state); composition and observers; configuration and types last. No virtual DOM is not an aspect: the Light & fast proof already says it. Bloom Field supplies the quiet
...
keyboards, scroller start/end alignment, grid toggle, the hero loop (stilled by a pointer on the plate and by focus in the frame, resumed after), reduced motion, and clipboard success / failure. Keep the
...
1. **The wordmark**: lowercase `spræ`, the `æ` ligature being the canonical poster form. The only lowercase display object on the page. The tridot mark, in the top bar and the footer, turns on its own (one turn in 40s), the favicon with it, and breathes under a pointer (see Atmosphere in the contract).
...
- sparse navigation opposite it (Usage, Reference, FAQ, capitalized), as labels, not buttons, each with the shared link rule (`--link-rule`, currentColor at 28%: a faint 1px underline `0.3em` below the text that fills on hover with no transition, one token for every text link on the page: menu, feature links, Start, FAQ, footer; every text link is also one weight step above its text, 500; stat numerals are links without an underline), with the version stamp, then npm's and GitHub's marks in one colour (`currentColor`; the octocat 24px, npm's own sharp-cornered square, faithful in its proportions, at 0.89 of that so it matches the circle's area), the stamp and both marks padded `--sp-1` on each side inside an `--sp-4` gap so the three visible gaps are equal (24px; `--sp-1` gap on phones), the last glyph pulled onto the shell's right edge: the stamp is the caption font at link weight (500, never mono) with the shared link rule, linking the package's version history on npm; the npm mark links the package, the GitHub mark the repository, and the last glyph sits on the shell's right edge (the tap target overhangs). Below 27rem the version stamp leaves the row, and below 22.5rem the npm mark does too (the footer keeps both links) while the links close up a step, so the row holds down to 320px. The stamp and the two pinned script sources must name the version npm serves; the footer's third column links npm, JSR (`@dy/sprae`), GitHub and Releases
...
- the plate is not editable: it shows one highlighted specimen and its live result. The hero line is the hook, a water-cycle wordplay on microhydration: “Sprinkle :attributes on your HTML –” / “sprae evaporates them, hydrating the DOM tree” (verbatim, en dash, lowercase sprae, no final stop) It is always exactly two lines (one sentence per line, no wrapping above `48rem`) at `--fs-2`. The cycle plays on its own in a loop through one `play` value and a `director()` effect in the hero scope (see the Hero contract for the loop, the tempo, and what stills it): the attribute names light up (names only, never their values); then the attributes alone evaporate: each directive-and-value group (`.a`, emitted by the highlighter with its leading space inside the group, so a closed group leaves no gap before the bracket) loses its highlight and blurs, fades, and closes up over 0.8s while the tags stay put, and stays closed; then hydration: the frame's own markup replaces the source and only what Sprae produced (`.w`: the value attribute, the rendered text) condenses in, growing from zero width as it sharpens, and the phrase is typed (`typewrite()`, 70ms a character). Evaporate and hydrate are distinct steps: the evaporate word alone empties the tags and inserts nothing; the hydrate word alone evaporates first, then hydrates. The cycle runs on 1.4s, 3.2s marks; after the saying has been read the values blur and shrink away and the attributes open up in place, and the cycle rests before playing again. Touch has no hover, and a tap fires pointerenter and pointerleave together, so touch pointers are ignored there (`fine(e)`). The whole source never blurs. Hovering one word plays only its step, and each hook word carries the active-style rule that grows from the left while its step runs. The phrase is not a control and shows no pointer cursor. Under each hook word run two underlines. A faint one (currentColor at 28%) fills first, at the plate's own transition speed (`--sync` on the word: the 300ms preview duration for attributes, the .8s evaporation or condensation for the other two), so it lands with the code. The solid one then fills over the whole step (`--step`: 1.4s attributes, 1.8s evaporate; .9s hydrate, the condensation of the values into the emptied tags), a progress bar for the step. Hydration ends with the values in place ("Hello."); the typing is a separate act after it: two seconds after hydration starts (so a second after the values have landed) the box clears and a saying is typed at 70ms per character, with no underline of its own. Both vanish when the step hands over; under reduced motion both simply show. Between cycles the plate crossfades back to the source. Source is a block, not an inline box, so the crossfade ghost never shifts it. The source area has a fixed minimum height for its four lines and is excluded from the swap's height animation, so nothing above it ever moves; the specimen carries no aria-label, and the live frame is exactly four major dot rows tall so it ends on the grid. No hint text under the specimen
...
- What the plate types after hydration is one of `window.sayings`, picked at random by `pick()` (never the one just shown) on load and at every hydration, the loop's and the hydrate word's alike, so every pass types a fresh one: aphorisms by sages on work, water, and life (Gita, Bhaktisiddhanta, Saint-Exupéry, Poincaré, Ramanujan, Jobs, Lao Tzu, Heraclitus, Marcus Aurelius), typed without attribution, just the phrase, and a few true facts about water and trees. Every line must be attributable or verifiable even though the source is not shown; no pop-science figures (the "humans share 50% of DNA with trees" claim is not one). Keep lines under about 80 characters so the typing ends within six seconds.
- The page loads sprae from unpkg, pinned to the version the page was made against (`https://unpkg.com/sprae@13.9.2/dist/sprae.umd.js`), and so does the live-result iframe's srcdoc; never from `dist/`: dist is gitignored, so on GitHub Pages it does not exist and the page would render unhydrated. Bump the pin together with the topbar version label.
- Entrance motion, the minimal set: on load the wordmark, statement, and hero line settle in once from `opacity 0, translate 0 8px, blur(6px)` over 1s `--ease-out`, staggered 150ms top to bottom; the plate does not animate, it is simply there. Not `@starting-style`: Chrome starts no transition from it for elements present at first paint. Instead an inline head script adds `pre` to the root and removes it two frames after DOMContentLoaded; `html.pre` holds the three texts back. Without JS the class is never added and the hero simply shows. The three features fade and rise 6px on entering the viewport (`:intersect.once` sets `on` on the section scope), staggered 60ms, and the stats strip fades 200ms after them, opacity only so its rule does not travel. The hidden state applies only after hydration (`:not([\:class])`, since sprae strips directive attributes), so a missing script never hides content; the transition belongs to the `.on` state only, so hydration hides instantly and never fades out. Reduced motion: opacity only, .3s, no blur, no travel. Nothing else on the page gets an entrance: tabs, reference, FAQ, footer stay still.
- The hero fills the first screen (`100svh` less the topbar) with its content bottom-aligned and lifted by `clamp(--sp-6, 5svh, --sp-12)` of bottom padding, so the plate ends about 4rem above the fold instead of on it. The features section keeps its overlap and pads only `--sp-3` on top, so the three icons start just under the fold (about 1rem below at 900px): nothing of Features shows on the first screen, and the gap from the plate to the icons is about 5rem, the hero's lift and little else.
