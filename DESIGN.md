...
- Source is deliberately formatted with shorter lines and wraps in place on desktop; on phones code never wraps: the plate, the Usage block, and the reference example scroll sideways under a thin translucent bar (`scrollbar-color: var(--plate-edge) transparent`), and the line-number gutter is hidden. Height follows content. Keep concise operational notes below.
...
- sparse navigation opposite it (Usage, Reference, FAQ, capitalized), as labels, not buttons, each with the shared link rule (`--link-rule`, currentColor at 28%: a faint 1px underline `0.3em` below the text that fills on hover with no transition, one token for every text link on the page: menu, feature links, Start, FAQ, footer; stat numerals are links without an underline), with a small version stamp in the caption font (regular, never mono) beside the GitHub mark, whose glyph sits on the shell's right edge (the tap target overhangs)
