...
The tridot mark turns on its own, one turn in 40s, in the top bar and the footer (inline SVG, `.mark`), and the favicon turns with it: the same three dots drawn to a 64px canvas at the mark's current animation angle, a step every 250ms while the tab is on screen (rAF), Safari keeping the static SVG. Under a pointer or focus the dots keep swinging in toward each other and back, the breathing mode of a triatomic molecule: a 1.6s oscillation (`--osc`, a registered number on keyframes) runs underneath all the time, and hover springs its amplitude (`--amp`, transitioned on the damped `linear()` spring `--ease-spring`, 16% overshoot, bezier fallback) from 0 to 1, leaving springs it back, so nothing snaps at either end; `--breath = 1 − amp · osc · 0.28` scales the group about the centre to 0.72 at the trough while each dot counter-scales and keeps its size. No rotation or turning icon under reduced motion.
